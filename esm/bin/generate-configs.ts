...
}): Promise<[config: Config, prefixForAutogeneratedIndex:string]> => {
...
const applyConfig = async ({
  ...
  const [config,packagePrefix] = await getConfigFromConfigFile({ repoName, packageName, packagePath });
...
if (packageName === configsPackageName) {
  ...
  /* eslint-disable-next-line functional/no-expression-statements*/
  await Promise.all(
    repoNames.map(async ({name : repoName, path:repoPath }) => {
      ...
      /* eslint-disable-next-line functional/no-expression-statements*/
      await Promise.all(
        subRepos.map(async ({ name: subRepoName, path: subRepoPath }) => {
