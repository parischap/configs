...
import { regExpEscape, type ReadonlyRecord } from './internal/shared-utils/utils.js';
...
const baseConfig = ({
  ...
  readonly sourceFilesEnv?: ReadonlyRecord;
}): Array<Config> =>
  defineConfig([
...
export const plainEslintConfig = ({
  tsconfigRootDir,
}: {
  readonly tsconfigRootDir: string;
}): Array<Config> => baseConfig({ tsconfigRootDir });

export const nodeEslintConfig = ({
  tsconfigRootDir,
}: {
  readonly tsconfigRootDir: string;
}): Array<Config> => baseConfig({ tsconfigRootDir, sourceFilesEnv: globals.nodeBuiltin });

/* We don't use any dom specifities in our client code because it must run on the server. It's all hidden away in preact */
export const browserEslintConfig = ({
  tsconfigRootDir,
}: {
  readonly tsconfigRootDir: string;
}): Array<Config> => baseConfig({ tsconfigRootDir });
