...
export const packageNameFromCWD = () => path.basename(path.resolve());

export const repoNameFromCWD = () => path.basename(path.dirname(path.dirname(path.resolve())));
...
export const deepMerge = <K extends string | symbol, V>(
  ...Rs: ReadonlyArray<ReadonlyRecord<K, V>>
): Record<K, V> => Rs.reduce(deepMerge2, {} as Record<K, V>);
