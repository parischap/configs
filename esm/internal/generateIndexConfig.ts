...
export default (
  prefixForAutogeneratedIndex: string,
...
const projectPath = '${sourceFolderName}';

const capitalizeFirstLetter = (s:string):string => s.charAt(0).toUpperCase() + s.slice(1);

const action = async () => {
  console.log(packageNameTag + "automatic '${indexTsFilename}' generation");
  const indexPath = join(projectPath, '${indexTsFilename}');
  ...
  if (noIndex || indexContent.startsWith(autoGeneratedFileWarning)) {
    const projectFiles = (
      ...
      .filter(
        ({ name, extension, relativePath }) =>
          ${JSON.stringify(javaScriptExtensions)}.includes(extension)
        ...
        + projectFiles
          ...
          .map(
            ({ bareName, capitalizedBareName, relativePath }) =>
              'export * as ${prefixForAutogeneratedIndex}'
              + capitalizedBareName
              + " from './"
    ...
    throw new Error(
      packageNameTag
        + "could not autogenerate '${indexTsFilename}' because the file already exists\
...
if (command === 'run')
  ...
  for await (const event of watcher) {
    ...
    if (currentEventTime - lastEventTime > 500) {
      ...
      if (changeFilename !== '${indexTsFilename}' && ${JSON.stringify(javaScriptExtensions)}.includes(extname(changeFilename))) {
        /* eslint-disable-next-line functional/no-expression-statements */
        await action();
      }
    }
