...
import { Package, type ReadonlyRecord, type Record, isArray, isRecord } from './types.js';
...
/** Returns all the packages of the TopRepo that contains the current path */
export const getAllPackages = async (): Promise<Array<Package>> => {
  ...
  return [
    { name: topRepoName, absolutePath: topRepoPath, type: 'TopRepo' },
    ...(
      await Promise.all(
        repoNames.map(async (repoName) => {
          ...
          const repo: Package =
            isMonoRepo ?
              { name: repoName, absolutePath: repoPath, type: 'MonoRepo' }
            : { name: repoName, absolutePath: repoPath, type: 'OnePackageRepo', isConfigsPackage };
          ...
          return [
            ...
            ...subRepos.map(
              (subRepoName) =>
                ({
                  name: subRepoName,
                  absolutePath: join(repoPackagesPath, subRepoName),
                  type: 'SubRepo',
                  repoName,
                }) satisfies Package,
            ),
