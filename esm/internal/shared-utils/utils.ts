...
import { autoGeneratedFileWarning, gitIgnoreFilename, javaScriptExtensions, prettierIgnoreFilename, ymlExtensions } from './constants.js';
...
/**
 * Same as node readdir but always returns Dirents. If `dontFailOnInexistentPath` is true, will not fail if `path` does not exist
 */
...
> => {
  try {
    ...
    if (dontFailOnInexistentPath && e instanceof Error && 'code' in e && e.code === 'ENOENT') return [];
    throw e;
  }
};
...
/**
 * Reads the contents of the `path` folder, recursively, but excludes the top folders whose list is given by `foldersToExclude`. If `dontFailOnInexistentPath` is true, will not fail if `path` does not exist. For each found file returns:
 *
 ...
 * - relativePath : the path to the the file expressed relatively to `relativePathSource` which is
...
>  => {
  ...
  const topFolders = topContents.filter((dirent)=>dirent.isDirectory()).filter(({name})=>!foldersToExclude.includes(name)).map(({name})=>name)
  const otherContents = await Promise.all(topFolders.map((topFolderName)=>
     readDir({path: join(path,topFolderName), recursive:true,dontFailOnInexistentPath: false})
    ))
  return [...topContents, ...otherContents.flat()].filter((dirent)=>dirent.isFile()).map(({ name, parentPath }) => {
...
/**
 * Writes the autogenerated `newContents` prefixed with 'autoGeneratedFileWarning' at path `path` only if the file does not exist or exists and starts with `autoGeneratedFileWarning`
 */
export const writeAutogeneratedFile = async ({path, newContents}:{readonly path:string, readonly newContents:string}):Promise<void>=>{
      const name = basename(path)
    const extension = extname(name)

    const commentType =  ymlExtensions.includes(extension) || name === prettierIgnoreFilename || name === gitIgnoreFilename ? 'Hashed' : javaScriptExtensions.includes(extension) ? 'Slashed' : 'NotSupported'
