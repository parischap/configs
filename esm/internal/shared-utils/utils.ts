...
/** Reads the command lines arguments */
export const getExeFlags = <A extends ReadonlyArray<string>>(
  ...
  const result = Array<boolean>(allowedFlagsNum).fill(false);
  for (const flag of args) {
    const index = allowedFlags.findIndex((allowedFlag) => flag === allowedFlag);
    if (index === -1)
      throw new Error(
        `Unexpected flag '${flag}' received. Allowed flags: '${allowedFlags.join("', '")}'`,
      );
    if (result[index] === true) throw new Error(`Flag '${flag}' received twice`);
