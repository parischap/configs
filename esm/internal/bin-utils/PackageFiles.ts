/**
 * This module represents an object whose keys are the names of the files to create and the values
 * the contents of these files. If the key has a `json` extension, the associated value is an object
 * that will be converted to a json string with JSON.stringfy. Otherwise, the associated value must
 * be a string.
 */
...
import { deepMerge, prettyStringify } from '../shared-utils/utils.js';
...
const sourcePackage = ({
  ...
  const buildConfig = (): Type => {
    ...
    throw new Error(`${tag}disallowed value for 'buildMethod' parameter. Actual: '${buildMethod}'`);
  };
  ...
  const platformConfig = () => {
    ...
    throw new Error(
      `${tag}disallowed value for 'useEffectPlatform' parameter. Actual: '${useEffectPlatform}'`,
    );
  };
  ...
  const environmentConfig = () => {
    ...
    throw new Error(`${tag}disallowed value for 'environment' parameter. Actual: '${environment}'`);
  };
