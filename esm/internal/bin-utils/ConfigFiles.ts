  ...
  (self: Type): Promise<Array<void>> =>
    Promise.all(
      Object.entries(self.configurationFiles).map(async ([filename, fileContent]) => {
        ...
        const commentType =
          isYml || filename === prettierIgnoreFilename || filename === gitIgnoreFilename
            ? 'Hashed'
            : jsoncExtensions.includes(ext)
              ? 'Slashed'
              : 'NotSupported';

        if (
          commentType === 'NotSupported'
        ) /* eslint-disable-next-line functional/no-expression-statements*/
        {
  ...
  make({
    ...(mode === Mode.Dev
      ? {
          ...
          // Used by the checks script
          [eslintConfigFilename]: EslintConfigPlain,
