...
const ESLINT_CONFIG_BROWSER_SOURCE = `import {browserEslintConfig} from '@parischap/configs/LinterConfig';
export default browserEslintConfig({tsconfigRootDir:import.meta.dirname})`;

const ESLINT_CONFIG_NODE_SOURCE = `import {nodeEslintConfig} from '@parischap/configs/LinterConfig';
export default nodeEslintConfig({tsconfigRootDir:import.meta.dirname})`;

const ESLINT_CONFIG_PLAIN_SOURCE = `import {plainEslintConfig} from '@parischap/configs/LinterConfig';
...
const VITE_CONFIG_LIB = ({
  buildMethod,
  isPublished,
}: {
  readonly buildMethod: string;
  readonly isPublished: boolean;
}) => `import {config} from '@parischap/configs/ViteLib';
export default config({ buildMethod: '${buildMethod}', isPublished: ${isPublished.toString()} });`;
      ...
      Object.entries(self.configurationFiles).map(async ([filename, fileContent]) => {
        ...
        const commentType =
          (
            ...
            || filename === formatterIgnoreFilename
            || filename === gitIgnoreFilename
          ) ?
            'Hashed'
          : javaScriptExtensions.includes(extension) ? 'Slashed'
          : 'NotSupported';
...
export const noSourcePackage: Type = make({
  ...
  // Used by the checks script
  [linterConfigFilename]: ESLINT_CONFIG_OTHERS,
...
const sourcePackageBuild = ({
  ...
  if (
    ...
    return make({
      [viteConfigFilename]: VITE_CONFIG_LIB({ buildMethod, isPublished }),
      [packageJsonFilename]: {
        sideEffects: [],
        scripts: {
          compile: 'vite build',
...
const sourcePackageEnvironment = ({
  ...
  const base = {
    ...
    [tsConfigOthersFilename]: {
      ...TSCONFIG_OTHERS,
      // The others project of the configs package needs to import the source project for  vitest.config.ts. See comment about TSCONFIG_TESTS for more details
  ...
  if (environment === 'Browser')
    return make({
      ...
      [linterConfigFilename]: ESLINT_CONFIG_BROWSER_SOURCE,
    });
  ...
  if (environment === 'Node')
    return make({
      ...
      // Used by the checks script
      [linterConfigFilename]: ESLINT_CONFIG_NODE_SOURCE,
    });
  ...
  if (environment === 'Plain')
    return make({
      ...
      // Used by the checks script
      [linterConfigFilename]: ESLINT_CONFIG_PLAIN_SOURCE,
    });
