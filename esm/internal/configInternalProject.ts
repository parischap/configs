  ...
  deepMerge(
    {
      ...
      [packageJsonFilename]: {
        ...
        devDependencies: {
          // Include self for tests. Don't use workspace protocol as one-package repos have no workspace
          // Use link so updates are reflected immediately
          [`${slashedScope}${packageName}`]:
            'link:.' /*`sourceInDev=WORKSPACE&buildStageInDev=DEV&parent=${repoName}`*/,
          /*
          ...
          : {
              [`${slashedScope}${testUtilsPackageName}`]:
                'workspace:*' /*'sourceInDev=AUTO&buildStageInDev=DEV'*/,
            }),
          ...projectDevDependencies,
          ...devDependencies,
        },
