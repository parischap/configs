...
All modules in a library need not be exported. The modules not to be exported must be put away under `esm/internal/`.

Any exported module must be made available to the client either as named import `import { MArray } from "@parischap/effect-lib";` and default import `import * as MArray from "@parischap/effect-lib/MArray";`.

A `index.ts` file in `esm/` must reexport all modules under `esm/` and not under `esm/internal`. This file could be generated during the build. However, it is very convenient to enable auto-completion of imports in vscode and this only works with named imports reexported in the `index.ts` file. So this file must be generated on the fly, either manually or by using vscode `TypeScript Barrel Generator` plugin.

A library can be public or private. A private library can be installed directly from github and does not need to be built: it is directly consumed in its Typescript version.
...
- Transform named imports after transpilation: Vscode will generate named imports during editing, but tree-shaking will the fail. So they must be changed to default imports during the build.
- Copy `esm/README.md` if there is one to `dist`.
- Generate a `LICENSE` file directly under `dist`.
- Copy `esm/package.json` to `dist`, change the package name to prod name, spread the `publishConfig`, remove specific keys (`scripts`, `devDependencies`, `publishConfig`, `packageManager`, `pnpm`, `devEngine`), remove keys with undefined values.
- Generate a `package.json` file under `dist/cjs` with a `commonjs` type.
- for each file under `esm/` and not under `esm/internal/` except `index.ts`, create a folder with a `package.json` for default imports and update the `exports` field of `package.json` accordingly.
