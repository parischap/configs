...
A `project` is a collection of repos gathered under a `top package`. This `top package` is entirely generated by the configs package. It is therefore not tracked by git. This `top package`:

- defines `pnpm` overrides so that internal packages are fetched locally instead of on `npm` : modification to internal packages are reflected immediately without the need for rebuilding and republishing. It makes development faster. A major drawback of this solution is that it does not force me to add meaningful comments at each commit. It is important to note that these overrides only take effect on local machines. They do not operate on `github` where no repo has been created for the top package;
- ensures coherence of installed packages in the development environment (`pnpm` tries to homogenize package installation with a workspace)
...
└───pnpm-workspace.yaml (pnpm workspace with overrides englobing all repos and sub-packages)
...
- optionally a `bin` subdirectory that contains modules that will not be exported, but that can be used as commands called directly in the node-modules directory (or installed by `pnpm` if they are references under the bin `package.json` key);
...
Each code-package contains `tests` directory that contains test modules for all its exported and non-exported modules. These test modules must import the package as if it were an external package

# Building a code package

There are three build modes:

- transpile: all modules in the `esm` directory, whether under the `internal` and `bin` subdirectories or not, are simply transpiled to JavaScript. Nothing gets bundled. If the package uses any dependencies, they must be installed in the prod package as well. This is usually a good choice for libraries.
...
Following is a list of some parameters to provide to the `configSubPackage` and `configOnePackageRepo` functions:
